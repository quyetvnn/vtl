...
/**
 * SchoolsGroups Controller
 */
class SchoolsGroupsController extends SchoolAppController {
	...
	public function api_get_item() {	// web
        ...
        if ($this->request->is('post')) {
			...
			if (!isset($data['language']) || empty($data['language'])) {
				...
				$temp = $this->SchoolsGroup->get_item($data['category_id'], $data['search_text'], $data['offset'], $data['limit'], $data['language']);
	...
	public function api_add_item() {	// web
        ...
        if ($this->request->is('post')) {
			...
			if (!isset($data['language']) || empty($data['language'])) {
				...
				$result = $this->SchoolsGroup->add_item($data['category_id'], $data['name'], $result_MemberLoginMethod['MemberLoginMethod']['member_id']);
	...
	public function api_edit_item() {	// web
        ...
        if ($this->request->is('post')) {
			...
			if (!isset($data['language']) || empty($data['language'])) {
				...
				if (!$this->SchoolsGroup->exists($data['group_id'])) {
					$status = 999;
					$message = __('Invalid group id');
					goto load_data_api;
				}
				
				$result = $this->SchoolsGroup->edit_item($data['group_id'], $data['name'], $data['language'], $result_MemberLoginMethod['MemberLoginMethod']['member_id']);
